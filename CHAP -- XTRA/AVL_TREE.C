/******** Create and Delete node from  AVL Binary TREE *******/
/*************************** AVL_TREE.C **********************/
#include<stdio.h>
# define F 0
# define T 1
...
rec *Binary_Tree (char , rec *, int *);
void Output(struct NODE *, int );
rec *Balance_Right_Heavy(rec *, int *);
rec *Balance_Left_Heavy(rec *, int *);
rec *DELETE(struct NODE *, rec *, int *);
rec *Delete_Element(rec *, char , int *);

/****** Function to insert an element into tree ************/

rec *  Binary_Tree (char Info, rec *Parent, int *H)
{
  rec *Node1,*Node2;
  if(!Parent)
  {
	Parent = (rec *) malloc(sizeof(rec));
	Parent->Info = Info;
...
void Output(rec *Tree,int Level)
{
	int i;
	if(Tree)
	{
...
rec * Balance_Right_Heavy(rec *Parent, int *H)
{
	rec *Node1, *Node2;
	switch(Parent->Flag)
	{
...
rec * Balance_Left_Heavy(rec *Parent, int *H)
{
	rec *Node1, *Node2;
...
/* Replace the node at which key is found with
			     last right key of a left child */

rec * DELETE(rec *R, rec *Temp, int *H)
{
	rec *Dnode = R;
...
rec * Delete_Element(rec *Parent, char Info, int *H)
{
	rec *Temp;
   if(!Parent)
    {
...
void main()
{
	...
	char Info ;
	char choice;
	rec *Tree = (rec *)malloc(sizeof(rec));
	Tree =  NULL;
